...
const IndexPage = () => {
  ...
  const onToggleStatus = React.useCallback(
    async (is_active: boolean, id: number) => {
      const { isConfirmed } = await MySwal.fire({
        title: `Yakin ingin ubah status hero dari ${
      ...
      if (isConfirmed) {
        toast.promise(
          customAxios.put(`${API_URL}/master/hero/${id}`, payload),
          {
            pending: {
              render: () => {
                return 'Loading';
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    await toast.promise(
      customAxios.post<{ data: HeroMaster; message: string; success: boolean }>(
        stringifyUrl({
          url: `${API_URL}/master/hero`,
        }),
        data
  ...
  const onSubmit2: SubmitHandler<IFormInput> = async (data) => {
    await toast.promise(
      customAxios.put<{ data: HeroMaster; message: string; success: boolean }>(
        stringifyUrl({
  ...
  const onClickDelete = React.useCallback(
    async (id: number) => {
      const { isConfirmed } = await MySwal.fire({
        title: 'Yakin ingin hapus hero ini?',
      ...
      if (isConfirmed) {
        toast.promise(customAxios.delete(`${API_URL}/master/hero/${id}`), {
          pending: {
            render: () => {
              return 'Loading';
            },
      ...
      <AnimatePage>
        <DashboardLayout superAdmin>
          <div className='mb-4 flex items-center justify-between'>
            <h1 className='text-3xl'>Data Hero</h1>
          ...
          <div className='flex items-center justify-center'>
            <PaginationComponent
              pageCount={heroes?.data.pagination.lastPage ?? 1}
